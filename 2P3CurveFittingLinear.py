# -*- coding: utf-8 -*-
"""
Created on Tue Aug 16 10:45:43 2022
...
name = "15-11_36_31, Spacing 50 Close VK second tests"
fmt = ".csv"
svname = "Spacing 50 Close"
...
solution = np.array([guess1, guess2])

guess_mast = np.array([guess1, guess2])
#print("guess =", guess)
...
try:
    plt.plot(xrange, fun(xrange, opt[0], opt[1]),
...
plt.ylim(lbnd, upbnd)
title = 'Curve Fitting Algorithm (Linear)' + ': ' + sigfigdelta
plt.title(title) # gives the plot a title
plt.legend()
plt.savefig(svname + "curvefitlineargraph.png", dpi=600)
plt.show()
